...
while score==0
    ...
    for c=1:stop-start
        ...
        signblock(c,:)=[row,col];
        % the difference is i calculate the distance from the initial to
        % current state
        Manhatan(1,c)=tempath(c,3)+abs(signa(c,1)-2)+abs(signa(c,2)-2)+abs(signb(c,1)-3)+abs(signb(c,2)-2)+abs(signc(c,1)-4)+abs(signc(c,2)-2)+abs(signblock(c,1)-4)+abs(signblock(c,2)-4);
